...
void putch(unsigned char byte) 
{
    ...
    //if TXIF is '0' register is full and if TXIF is '1' register is empty then only we can assign new data of (1 byte)
	TXIF = 0;
	TXREG = byte;
} 
...
unsigned char getch(void)
{
    ...
    //if RCIF is '0' register is empty and if RCIF is '1' register full it will return the data
	RCIF = 0;
	return RCREG;
}
...
unsigned char getche(void)
{
    ...
    //   receive (1 byte data)
	putch(c = getch());
    //transmit that recieved (1 byte data)
	return (c);
}
